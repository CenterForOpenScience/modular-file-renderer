...
from .__init__ import IPynbRenderer
...
class TestIpynb(unittest.TestCase):
    def setUp(self):
        self.renderer = IPynbRenderer()

    #Do detect

    def test_detect_ipynb(self):
        file_pointer = open(os.path.join(here, 'fixtures/test.ipynb'))
        detected = self.renderer._detect(file_pointer)
        assert_true(detected)

    # Do not detect

    def test_dont_detect_docx(self):
