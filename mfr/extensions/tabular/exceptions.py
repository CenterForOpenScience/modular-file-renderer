...
class MissingRequirementsError(TabularRendererError):
    ...
    def __init__(
        self, message, *args, code: int = 500, function_preference: str = "", **kwargs
    ):
        super().__init__(message, *args, code=code, **kwargs)
        self.function_preference = function_preference
        self.attr_stack.append(
            [self.__TYPE, {"function_preference": self.function_preference}]
        )
...
class TableTooBigError(TabularRendererError):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attr_stack.append(
            [self.__TYPE, {"nbr_cols": self.nbr_cols, "nbr_rows": self.nbr_rows}]
        )
...
class UnexpectedFormattingError(TabularRendererError):
    ...
    def __init__(
        self, message, *args, code: int = 500, formatting_function: str = "", **kwargs
    ):
        super().__init__(message, *args, code=code, **kwargs)
        self.formatting_function = formatting_function
        self.attr_stack.append(
            [self.__TYPE, {"formatting_function": self.formatting_function}]
        )
...
class FileTooLargeError(TabularRendererError):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attr_stack.append(
            [self.__TYPE, {"file_size": self.file_size, "max_size": self.max_size}]
        )
