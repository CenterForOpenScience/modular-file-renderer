...
async def log_analytics(request, metrics, is_error=False):
        ...
        keen_payload["keen"]["addons"].append(
            {
                ...
                "input": {"referrer_url": "referrer.url", "page_url": "request.url"},
    ...
    if (is_error and settings.KEEN_PRIVATE_LOG_ERRORS) or settings.KEEN_PRIVATE_LOG_VIEWS:
        await _send_to_keen(
            keen_payload,
            private_collection,
            settings.KEEN_PRIVATE_PROJECT_ID,
            settings.KEEN_PRIVATE_WRITE_KEY,
            keen_payload["handler"]["type"],
            domain="private",
        )

    if (
        keen_payload["handler"]["type"] != "render"
        or file_metadata is None
        or is_error
        or not settings.KEEN_PUBLIC_LOG_VIEWS
    ):
        return
    ...
    await _send_to_keen(
        public_payload,
        "file_stats",
        settings.KEEN_PUBLIC_PROJECT_ID,
        settings.KEEN_PUBLIC_WRITE_KEY,
        keen_payload["handler"]["type"],
        domain="public",
    )
...
async def _send_to_keen(payload, collection, project_id, write_key, action, domain="private"):
    ...
    url = "{}/{}/projects/{}/events/{}".format(
        settings.KEEN_API_BASE_URL, settings.KEEN_API_VERSION, project_id, collection
    )
...
def _scrub_headers_for_keen(payload, MAX_ITERATIONS=10):
    ...
    for key in sorted(payload):
        ...
        if scrubbed_key in scrubbed_payload:
            for i in range(
                1, MAX_ITERATIONS + 1
            ):  # try MAX_ITERATION times, then give up & drop it
