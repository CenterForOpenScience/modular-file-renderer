...
def metadata():
    return ProviderMetadata(
        ...
        "text/plain",
        "1234",
        "http://wb.osf.io/file/good.xml?token=1234",
    )
...
class TestCodePygmentsRenderer:
    ...
    def test_render_codepygments_max_size(
        self, metadata, max_size_file_path, url, assets_url, export_url
    ):
        try:
            renderer = CodePygmentsRenderer(
                metadata, max_size_file_path, url, assets_url, export_url
            )
            body = renderer.render()
            assert '<div style="word-wrap: break-word;" class="mfrViewer">' in body
        finally:
    ...
    def test_render_codepygments_over_size(
        self, metadata, over_size_file_path, url, assets_url, export_url
    ):
        with pytest.raises(FileTooLargeError):
            try:
                renderer = CodePygmentsRenderer(
                    metadata, over_size_file_path, url, assets_url, export_url
                )
                renderer.render()
            finally:
    ...
    def test_render_encodings(
        self, metadata, invalid_file_path, url, assets_url, export_url, filename
    ):
