...
def metadata():
    return ProviderMetadata(
        ...
        "text/plain",
        "1234",
        "http://wb.osf.io/file/test.ipynb?token=1234",
    )
...
def test_file_path():
    return os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "files", "test.ipynb"
    )
...
def no_metadata_file_path():
    return os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "files", "no_metadata.ipynb"
    )
...
def invalid_json_file_path():
    return os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "files", "invalid_json.ipynb"
    )
...
class TestIpynbRenderer:
    ...
    def test_render_ipynb_no_metadata(
        self, metadata, no_metadata_file_path, url, assets_url, export_url
    ):
        renderer = IpynbRenderer(
            metadata, no_metadata_file_path, url, assets_url, export_url
        )
        body = renderer.render()
        assert (
            '<div style="word-wrap: break-word;" class="mfrViewer mfr-ipynb-body">'
    ...
    def test_render_ipynb_invalid_json(
        self, metadata, invalid_json_file_path, url, assets_url, export_url
    ):
        renderer = IpynbRenderer(
            metadata, invalid_json_file_path, url, assets_url, export_url
        )
